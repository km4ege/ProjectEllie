...
int HMC5883L::setup(int scale, int measurement){
    int error;
    error = SetScale(scale); //Set the scale of the compass.
    error = SetMeasurementMode(measurement); // Set the measurement mode to Continuous
    return error;
}
void HMC5883L::readAxis()
{
  uint8_t* buffer = Read(DataRegisterBegin, 6);
  rawX = (buffer[0] << 8) | buffer[1];
  rawY = (buffer[2] << 8) | buffer[3];
  rawZ = (buffer[4] << 8) | buffer[5];
  x = rawX * m_Scale;
  y = rawZ * m_Scale;
  z = rawY * m_Scale;
}
